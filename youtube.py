...
def get_video_id(url):
    patterns = [
        ...
        r'(?:live\/|live\?.+&v=)([0-9A-Za-z_-]{11})',  # Include live stream pattern
        r"shorts\/(\w+)" # Include shorts pattern
    ]
...
def get_playlist_videos(playlist_url):
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if playlist_info and 'entries' in playlist_info:  # Check if entries exist
            return [entry['url'] for entry in playlist_info['entries'] if entry] # filter out None entries
...
def main(urls):
    ...
    for video_url in video_urls:
        ...
        if not video_id:
            print(f"Invalid URL or unable to extract video ID: {video_url}")
            continue
